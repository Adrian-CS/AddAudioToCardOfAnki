...
| オプション | 説明 |
|---|---|
| **既存の音声を上書き** | フィールドにすでにサウンドタグがある場合でも再取得する |
| **TTS をフォールバックとして使用** | Wiktionary にネイティブ録音がない場合、Google 翻訳 TTS で音声を生成する |
...
音声が見つからないカードは、英語の単語フィールド、複数単語の説明文、または Wiktionary にまだ録音がない単語である場合がほとんどです。
